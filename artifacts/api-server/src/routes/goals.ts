...
router.get("/goals", async (req, res) => {
  ...
  const rows = await db.select().from(goalsTable).where(eq(goalsTable.userId, getUserId(req)));
  const result = rows.filter((g) => !category || g.category === category);
  return res.json(result);
});

router.post("/goals", async (req, res) => {
  const parsed = insertGoalSchema.safeParse({ ...req.body, userId: getUserId(req) });
  if (!parsed.success) return res.status(400).json({ error: parsed.error.message });
  const inserted = await db.insert(goalsTable).values(parsed.data).returning();
  const activeCount = await db.select().from(goalsTable).where(and(eq(goalsTable.userId, getUserId(req)), eq(goalsTable.isCompleted, false)));
  await db.update(usersTable).set({ goalsActive: activeCount.length }).where(eq(usersTable.id, getUserId(req)));
