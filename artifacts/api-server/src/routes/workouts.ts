...
async function sessionWithBlocks(id: number) {
  const rows = await db
    .select()
    .from(workoutSessionsTable)
    .where(and(eq(workoutSessionsTable.id, id), eq(workoutSessionsTable.userId, DEFAULT_USER_ID)))
    .limit(1);
...
router.get("/workouts", async (req, res) => {
  ...
  const conditions = [eq(workoutSessionsTable.userId, DEFAULT_USER_ID)];
...
router.get("/workouts/:id", async (req, res) => {
  ...
  const session = await sessionWithBlocks(id);
  if (!session) return res.status(404).json({ error: "Workout not found" });
  return res.json(session);
});

router.post("/workouts", async (req, res) => {
  const parsed = insertWorkoutSessionSchema.safeParse({ ...req.body, userId: DEFAULT_USER_ID });
  if (!parsed.success) return res.status(400).json({ error: parsed.error.message });
  const inserted = await db.insert(workoutSessionsTable).values(parsed.data).returning();
  const session = await sessionWithBlocks(inserted[0].id);
...
router.post("/workouts/:id/blocks", async (req, res) => {
  const id = Number(req.params.id);
  const owner = await sessionWithBlocks(id);
